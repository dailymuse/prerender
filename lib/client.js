...
function PrerenderClient(phantom) {
    ...
    phantom.on("request", _.bind(this.onQueueItemReady, this));
}
...
PrerenderClient.prototype.onQueueItemReady = function(page, item) {
    var _this = this;
    ...
    _this._pluginEvent("onPhantomPageCreate", [page, context], function() {
        ...
        page.open(context.item.request.url, function(status) {
...
PrerenderClient.prototype.onResourceReceived = function (page, context, response) {
    ...
    if(normalizeUrl(context.item.request.url) === normalizeUrl(response.url) && response.status >= 300 && response.status <= 399) {
    ...
    if ('end' === response.stage) { 
        ...
        if (normalizeUrl(context.item.request.url) === normalizeUrl(response.url)) {
            context.pendingRequests--;
...
PrerenderClient.prototype.respond = function(page, context, abort) {
    ...
    context.item.finish(context.response);
